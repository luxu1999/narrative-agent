...
// 中文 1 字 ≈ 1~2 token，按 4 倍预留余量（含模型 thinking 占用、标点/分段开销）
// 封顶 16000：支持最大约 3000~4000 字长文（3000字×4=12000 < 16000）；下限 500
// 注：v0.3.29 用 2.5 倍导致带思考的模型正文被截断（用户反馈「然后继续」半截句），改为 4 倍；封顶原 8000 只能支持 ~2000 字，应主人要求提至 16000
export function resolveResponseLength(ctx) {
  const max = Number(ctx.maxReplyChars);
  if (max <= 0) return null;
  return Math.min(16000, Math.max(500, Math.ceil(max * 4)));
}
