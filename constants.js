...
- 重要记忆点（累积制，长期记忆，会取舍）：上一状态追踪中的全部记忆必须无条件完整保留，仅当本轮发生值得记录的新事件时追加；每角色最多6条，**超过6条时必须舍弃相对最不重要的一条，保留最重要的6条**（重大事件如救命/死亡/背叛/第一次/告白/怀孕优先保留，日常琐事优先舍弃）；即使本轮没有任何新记忆，也必须完整输出上一状态的全部记忆（禁止漏掉、禁止重写）；每角色6条合计≤70字
