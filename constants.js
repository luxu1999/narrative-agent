...
- 当前态度：女性角色当前**表面表现**的态度/情绪，格式「角色：L级别+名称（可选括号标注）」，如：琴：L14服软（受胁迫）、芭芭拉：L18讨好（内心L2敌视，贪婪图谋）。**20级态度表**（级别/名称/绑定好感度档）：L1杀意、L2敌视（-100~-81仇敌）|L3厌恶、L4冷拒（-80~-61厌恶）|L5对抗、L6疏远（-60~-41反感）|L7冷淡、L8不满（-40~-21冷淡）|L9警惕、L10戒备（-20~-1警惕）|L11中立、L12缓和（0~20中立）|L13友好、L14服软（21~40友好）|L15信任、L16顺从（41~60信任）|L17依赖、L18讨好（61~80依赖）|L19依恋、L20痴缠（81~100深爱）。**表面与内心**：表面态度=角色表现出的言行态度，由剧情与动机驱动；好感度=角色真实的内心情感。两者允许不一致——受胁迫、贪婪图谋、虚伪利用、策略讨好等动机驱动的表里不一是合理且值得描写的别扭剧情，不要强行改回。**括号标注**：表里不一致时必须注明原因：①「内心Lxx名称」注明真实态度（如：琴：L18讨好（内心L2敌视））②或注明动机（如「受胁迫」「图谋某物」「虚伪敷衍」）；表里一致时括号可省略。**规则**：①内心真实态度（若标注）应与好感度档位一致：好感度-100的内心应落在L1~L2区间，好感度85的内心应落在L19~L20区间；表面态度不受此限制②无新事件严格复制上一轮态度（含括号标注），禁止态度无故反复横跳（上轮服软这轮突然变强硬且无事件驱动）③±1级微调由情境事件驱动（吵架升级/道歉缓和等）④跨档位转变必须伴随明确事件或动机变化（如胁迫解除、图谋得逞）⑤暂时/伪装等标注的添加与解除必须由明确事件驱动（胁迫场景→受胁迫、欺骗场景→伪装、事件结束→标注解除回归）⑥态度变化属于状态变化，必须在正文中有所体现，不能只改字段不写内容⑦该字段必须列出所有已追踪角色（在场+不在场）的表面态度
