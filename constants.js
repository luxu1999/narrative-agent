...
- 当前态度：女性角色当前的态度/情绪状态，格式「角色：L级别+名称（可选修饰符）」，如：琴：L14服软（暂时，受胁迫）。**20级态度表**（级别/名称/绑定好感度档）：L1杀意、L2敌视（-100~-81仇敌）|L3厌恶、L4冷拒（-80~-61厌恶）|L5对抗、L6疏远（-60~-41反感）|L7冷淡、L8不满（-40~-21冷淡）|L9警惕、L10戒备（-20~-1警惕）|L11中立、L12缓和（0~20中立）|L13友好、L14服软（21~40友好）|L15信任、L16顺从（41~60信任）|L17依赖、L18讨好（61~80依赖）|L19依恋、L20痴缠（81~100深爱）。**修饰符**：默认真诚（可省略）；「暂时」=情境性态度、权宜之计（如受胁迫服软），不改变好感度，事件解除后回归档位；「伪装」=表面如此内心相反，必须注明真实态度（如：琴：L14服软（伪装，内心L2敌视）），不改变好感度。**规则**：①无修饰符时态度级别必须落在好感度对应档位的2级区间内，禁止「好感度极低却态度温和」（如好感度-90却写L14服软）或「好感度极高却态度恶劣」的矛盾②无新事件严格复制上一轮态度（含修饰符），禁止态度无故反复横跳（上轮服软这轮突然变强硬且无事件驱动）③±1级微调由情境事件驱动（吵架升级/道歉缓和等）④跨档位转变（±2级以上）必须伴随好感度变化或重大事件（背叛/救命/告白等）⑤暂时/伪装的添加与解除必须由明确事件驱动（胁迫场景→暂时、欺骗场景→伪装、事件结束→解除修饰回归档位）⑥态度变化属于状态变化，必须在正文中有所体现，不能只改字段不写内容
