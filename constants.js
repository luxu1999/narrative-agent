...
当前好感度：女性角色对主角的好感度（-100~+100，如：琴：75（喜欢）、芭芭拉：-30（冷淡））
...
- 处女膜状态：仅当明确发生破处性交才更新
- 做爱次数：仅当明确发生性交且射精才+1
- 当前好感度（-100~+100 系统，容易掉难涨）：数值范围 -100~+100，可正可负；**评级掉落比增加更容易**——负面事件/言行导致好感下降（掉落），正面事件/言行导致好感上升（增加），但增加需要更充分、更明确的正面证据，掉落则只需较弱的负面证据。评级分档：-100~-81 仇敌/憎恨 | -80~-61 厌恶/敌视 | -60~-41 反感/疏远 | -40~-21 冷淡/不满 | -20~-1 警惕/不信任 | 0~20 陌生/中立 | 21~40 普通/友好 | 41~60 信任/亲近 | 61~80 依赖/喜欢 | 81~100 深爱/忠诚。典型影响：重大背叛/伤害/欺骗 -15~-30；日常不快/冷淡 -3~-10；日常友好/互动 +1~+5；重大恩情/告白/拯救 +8~+20。**注意：好感度仅当角色言行明确体现变化时才更新，无明确证据严格复制上一状态；同档位内数值可以微调，但跨档位必须对应明显事件**
- 身体外貌：仅当明确描写换装/脱衣/发型变化才更新，严禁凭空写裸体或换衣
- 重要记忆点（累积制，长期记忆，会取舍）：上一状态追踪中的全部记忆必须无条件完整保留，仅当本轮发生值得记录的新事件时追加；每角色最多6条，**超过6条时必须舍弃相对最不重要的一条，保留最重要的6条**（重大事件如救命/死亡/背叛/第一次/告白/怀孕/性格心态改变优先保留，日常琐事优先舍弃）；即使本轮没有任何新记忆，也必须完整输出上一状态的全部记忆（禁止漏掉、禁止重写）；每条≤20字，每角色合计≤120字，禁止故意凑字数；**能改变角色性格、心态的事件（如开朗→沉默寡言）属于重大事件，评分应与人生转折同级，改变程度越大评分越高**
