...
每轮合并分析 Agent 将本轮对话压缩为摘要追加到列表，共两条：
- [第N轮]状态追踪：… — 世界状态追踪（9 字段：时间/区域/在场角色+BUFF/不在场角色/处女膜状态/做爱次数/当前好感度/身体外貌/重要记忆点），最小变化原则，仅在有明确证据时更新；随消息/checkpoint 一起走（删除消息时回滚到对应轮次，状态不会错位）
- [第N轮] 叙事要点：yyy — 故事核心事实摘要

上一轮状态追踪会自动注入下一轮合并分析作为状态演化基准。
...
```json
{
  ...
  "summary_entries": ["[第3轮] 叙事要点：兰韦德进入矿洞..."]
}
```
